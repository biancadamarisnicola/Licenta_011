...
import clasification.ann.ControllerClasif;
import common.Layer;
 ...
 */
public class MainClasification {
    private JFrame frame;
    private ControllerClasif ctrl;
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    MainClasification window = new MainClasification();
    ...
    public MainClasification() throws Exception {
        this.noHidden = 2;
...
//        textFieldLimitSEE.setColumns(10);
        ...
        for (Map.Entry stock : Stocks.entrySet()) {
            ...
            btnCalculate.addActionListener(arg0 -> {
                ...
                try {
                    ctrl = new ControllerClasif(stock.getValue().toString(), noEpoch, epsilon, learningRate, limitSEE,
                            adaptiveLR, alpha, noHidden, noNeurPerHidden);
                    Layer out = ctrl.getOutput();
                    s = String.valueOf(out.getNeuron(0).getOutput()+" "+out.getNeuron(1).getOutput()+" "+
                            out.getNeuron(2).getOutput());
                } catch (IOException e) {
                    e.printStackTrace();
