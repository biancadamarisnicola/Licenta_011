...
public class NetworkClasif {
    ...
    public NetworkClasif(int noOfFeatures, int noOfOuputs, int noOfHidden, int noOfNeuronsPerLayer, Double epsilon,
    ...
    public void learn(Fraction[][] input, Fraction[][] output) {
        ...
        while ((!stopCondition) && (epoch < this.noEpoch)){
            ...
            for (int inputNo = 0; inputNo < input.length; inputNo++){
                ...
                globalError[inputNo] = errorComputationClasification(output[inputNo], err);
...
//                +" "+globalError[2].recalibreaza().recalibreaza().div(new Fraction().valueOf(noOfInputs))+" ");
    ...
    private Fraction errorComputationClasification(Fraction[] target, Fraction[] err) {
